...
namespace Tim
{
    ...
    [CustomEditor(typeof(MyPhotonAnimatorView))]
    public class MyPhotonAnimatorViewEditor : MonoBehaviourPunEditor
    {
        private Animator m_Animator;
        private MyPhotonAnimatorView m_Target;
        private AnimatorController m_Controller;

        /// <summary>
        /// InspectorGUI自動更新(Editor.cs)
        /// </summary>
        public override void OnInspectorGUI()
        {
            ...
            /*
            应用修改项於畫面，通常會自動執行，但還是預防萬一要自己執行
            https://www.jianshu.com/p/fde1ea2c0337 
        ...
        {
            this.m_Target = (MyPhotonAnimatorView)this.target;
        ...
        private void DrawWeightInspector()
        {
            ...
            for (int i = 0; i < this.GetLayerCount(); ++i)
            {
                ...
                {
                    /*加入內容 Disabled*/
                    this.m_Target.SetLayerSynchronized(i, MyPhotonAnimatorView.SynchronizeType.Disabled);
                }
                MyPhotonAnimatorView.SynchronizeType syncType = this.m_Target.GetLayerSynchronizeType(i);
                ...
                syncType = (MyPhotonAnimatorView.SynchronizeType)EditorGUI.EnumPopup(popupRect, syncType);
        ...
        private void DrawParameterInspector()
        {
            ...
            for (int i = 0; i < this.GetParameterCount(); i++)
            {
                ...
                {
                    /*加入內容 Disabled*/
                    this.m_Target.SetParameterSynchronized(parameter.name, (MyPhotonAnimatorView.ParameterType)parameter.type, MyPhotonAnimatorView.SynchronizeType.Disabled);
                }

                MyPhotonAnimatorView.SynchronizeType value = this.m_Target.GetParameterSynchronizeType(parameter.name);

                // check if using trigger and actually synchronizing it
                if (value != MyPhotonAnimatorView.SynchronizeType.Disabled && parameter.type == AnimatorControllerParameterType.Trigger)
                {
                ...
                Rect popupRect = new Rect(elementRect.xMin + EditorGUIUtility.labelWidth, elementRect.yMin + 2, elementRect.width - EditorGUIUtility.labelWidth - 5, EditorGUIUtility.singleLineHeight);
                value = (MyPhotonAnimatorView.SynchronizeType)EditorGUI.EnumPopup(popupRect, value);
                ...
                if (value != this.m_Target.GetParameterSynchronizeType(parameter.name))
                {
                    ...
                    this.m_Target.SetParameterSynchronized(parameter.name, (MyPhotonAnimatorView.ParameterType)parameter.type, value);
                }
            }
